...
public class StatisticsService {
    ...
    public StatisticsService(LibraryService libraryService, HistoryService historyService) {
    ...
    public int getTotalSongs() {
        return libraryService.count();
    }

    public double getAverageRating() {
        return libraryService.listAll().stream().mapToInt(Song::getRating).filter(rating -> rating > 0).average().orElse(0.0);
    }
    ...
    public List<StatsDto.ArtistCountDto> getTopArtists(int n) {
        ...
        return libraryService.listAll().stream()
                ...
                limit(n).map(entry -> new StatsDto.ArtistCountDto(entry.getKey(), entry.getValue().intValue())).toList();
    }
    ...
    public List<Song> getMostPlayedSongs(String userId, int n) {
        ...
        return historyService.listHistory(userId).stream()
                .collect(Collectors.groupingBy(HistoryEntry::getSongId, Collectors.counting())).entrySet().stream()
                .sorted(Map.Entry.<String, Long>comparingByValue().reversed().thenComparing((Map.Entry.comparingByKey())))
                .limit(n).map(entry -> libraryService.getSongById(entry.getKey())).filter(Objects::nonNull).toList();
    }
